...
// cheating : look at the data and determine the rx's ancestors
var ancestors = ["pm","mk","pk","hf"];
var total = 1;
for(var i=0;i<ancestors.length;i++) {
	var n = 1;
	reset();
	while(!run(mindex(ancestors[i])))
		n++;
	total *= n;
}
...
function run(target) {
	...
	while(pulses.length>0) {
		var p = pulses.shift();
		if((p[1]==target)&&(p[2]==LO)) return true;
		count[p[2]]++;
		if(p[1]>=0)
			modules[p[1]].rcv(p);
	}
